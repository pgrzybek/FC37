...
from OperacjeNaPliku import odczytajKonto, odczytajMagazyn, odczytajOperacje
...
class Magazyn:
    ...
    def lista(self):
        for jednostka in self.skladMagazynu:
            ...
            return self.skladMagazynu[jednostka].wypisz()
    ...
    def przeglad(self, zakres_start, zakres_koniec):
        if zakres_start == "" or zakres_koniec == "":
            ...
            if zakres_start > 0 and zakres_koniec < len(self.operacje):
                for i in range(zakres_start, zakres_koniec):
                    self.operacje[i].wypisz()
                return None
            else:
                print("zly zakres")
                return "zly zakres"
